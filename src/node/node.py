...
class NodeLogic:
    ...
    def train(self):
        """Effectuer la formation locale sur le nœud."""
        mm = MetricManager(self.args.eval_metrics)
        self.model.train()
        optimizer = self.optim(self.model.parameters(), **self._refine_optim_args(self.args))
        ...
        mm.aggregate(len(self.node_dataset))
        return mm.results

    def evaluate(self):
        """Évaluer le modèle local."""
        mm = MetricManager(self.args.eval_metrics)
        self.model.eval()
        with torch.no_grad():
            for inputs, targets in self.test_loader:
    ...
    def _create_dataloader(self, dataset, shuffle):
        if self.args.B == 0:
            self.args.B = len(dataset)
        return torch.utils.data.DataLoader(dataset, batch_size=self.args.B, shuffle=shuffle)

    def _refine_optim_args(self, args):
        required_args = inspect.getfullargspec(self.optim)[0]
        return {k: getattr(args, k) for k in required_args if hasattr(args, k)}
